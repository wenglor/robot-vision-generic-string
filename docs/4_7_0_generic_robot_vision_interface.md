...
| calibration:add[*pose_information*]; | Triggers and saves the current camera image together with the given robot pose in the internal robot server buffer. | Pose information in [x, y, z, rx, ry, rz] x, y, z in meter rx, ry, rz as rotation vector in radians |
